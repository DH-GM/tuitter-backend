...
from sqlalchemy import (
    String, Text, Integer, Boolean, ForeignKey, TIMESTAMP, Index, create_engine, func
)
...
class User(Base):
    ...
    last_seen_at: Mapped[dt.datetime] = mapped_column(TIMESTAMP(timezone=True), server_default=func.now())
...
class Post(Base):
    ...
    created_at: Mapped[dt.datetime] = mapped_column(TIMESTAMP(timezone=True), server_default=func.now(), index=True)
...
class Notification(Base):
    ...
    related_post_id: Mapped[str | None] = mapped_column(ForeignKey("posts.id"), nullable=True)
    created_at: Mapped[dt.datetime] = mapped_column(TIMESTAMP(timezone=True), server_default=func.now(), index=True)
    read: Mapped[bool] = mapped_column(Boolean, default=False)

_ = Index("idx_notifications_user_time", Notification.user_id, Notification.created_at.desc())
