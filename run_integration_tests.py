...
def req(method, path, *, params=None, json_body=None):
    url = f"{BASE_URL}{path}"
...
def main():
    ...
    settings_a2 = req("GET", "/settings", params={"handle": user_a})
    assert_true(
    ...
    comments = req("GET", f"/posts/{post_id}/comments")
    assert_true(
    ...
    disc = req("GET", "/discover", params={"handle": user_a, "limit": 10})
    assert_true(isinstance(disc, list), "Discover returns array")

    print("=== C O N V E R S A T I O N S  &  M E S S A G E S ===")
    dm = req(
        "POST", "/dm", json_body={"user_a_handle": user_a, "user_b_handle": user_b}
    )
    conv_id = dm.get("id")
    ...
    msgs = req("GET", f"/conversations/{conv_id}/messages")
